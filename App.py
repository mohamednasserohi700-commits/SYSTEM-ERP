...
def get_next_number(prefix, model, field):
    ...
    ✅ يعمل بأمان على SQLite و PostgreSQL على Railway
    """
    try:
        # ✅ استخدم model.id مباشرة - آمن على جميع قواعد البيانات
        last = db.session.query(model).order_by(model.id.desc()).first()
        num = (last.id + 1) if last else 1
    except Exception as e:
        print(f"⚠️  خطأ في get_next_number: {e}")
        # احتياطي: في حالة نادرة جداً، استخدم timestamp
        import time
        num = int(time.time()) % 1000000
